...
/*  ... posting to left that rcv_buf is exposed to left, i.e.,
    ...
    volatile_store(signal_A_ptr+(left-my_rank_sm), 1);
...
/*  rcv_buf is now writable by the origin process */     
    ...
    *(rcv_buf_ptr+(right-my_rank_sm)) = snd_buf; /* the origin process writes into the target rcv_buf */
...
/*  ... posting to right that rcv_buf was stored from left */
    volatile_store(signal_B_ptr+(right-my_rank_sm), 1);
